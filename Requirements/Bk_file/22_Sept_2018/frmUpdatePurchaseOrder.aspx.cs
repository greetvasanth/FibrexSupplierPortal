        ...
        protected void btnAddLines_Click(object sender, EventArgs e)
        {
            ...
            if (dt.Rows.Count == 1)
            {
                if (dt.Rows[0]["POLINEID"].ToString() == "")
                {
                    ...
                    dt.Rows[0]["TAXED"] = "False";
                    dt.Rows[0]["TAXCODE"] = "VAT";
                    dt.Rows[0]["TAXRATE"] = "5";
                    dt.Rows[0]["ADDEDBY"] = UserName;
                    dt.Rows[0]["ADDEDON"] = DateTime.Now.ToString();//ActionTaken
                    dt.Rows[0]["ActionTaken"] = "NEWLINE";//FillFeildsinTable("TAXRATE", rowIndex, TAXRATE, null);
                    lblpolineid.Text = "-1";
                    lblrowindex.Text = "0";
                ...
                if (Request.QueryString["ID"] != null)
                {
                    ...
                    if (ObjPO != null)
                    {
                       ...
                       string InternalNotes = null;
                        ...
                        Nullable<bool> SendtoAccounts = null;
                    ...
                    if (isChecked)
                    {
                        ...
                        if (UsrSec != null)
                        {
                            ...
                            if (UsrPOSignature == null)
                            {
                                try
                                {
                                    var masg = db.PO_ADDPOSignature(UsrSec.OrgCode, null, UsrSec.OrderNo, decimal.Parse(txtSignaturePONum.Text), Int16.Parse(txtSignaturePORevision.Text), UsrSec.Authority, UsrSec.Designation.ToString(), null, null, UserName, true);
                                }
                                catch (SqlException ex)
                                {
                                    lblSignatureError2.Text = ex.Message;
