...
namespace FibrexSupplierPortal.Mgment
{
    ...
    public partial class frmUpdatePurchaseOrder : System.Web.UI.Page
    {
        ...
        public void bindGrid(DataTable dt)
        {
            ...
            {
                ((CheckBox)grd.HeaderRow.FindControl("chkCC")).Visible = true;
            ...
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                if (rowID != "")
                {
                    if (e.Row.RowIndex != grd.EditIndex)
                    {
                        ...
                        //e.Row.Attributes.Add("onkeypress", "javascript:if (event.keyCode == 13) {__doPostBack('" + grd.UniqueID + "', 'Update$" + e.Row.RowIndex.ToString() + "'); return false; }");
                        //e.Row.Attributes["onkeypress"] = "javascript:if (event.keyCode == 13) {__doPostBack('" + grd.UniqueID + "', 'Update$" + e.Row.RowIndex.ToString() + "'); return false; }";
                    }
                }
            }
        ...
        protected void btnPaste_Click(object sender, EventArgs e)
        {
            ...
            mydiv.Visible = false;
            grd.EditIndex = -1;
            DataTable dt1 = (DataTable)ViewState["PoLines"];
            bindGrid(dt1);

            txtCopied.Text = "";
            txtCopied.Visible = false;
            ((CheckBox)grd.HeaderRow.FindControl("chkCC")).Visible = false;
        ...
        protected void btnAddLines_Click(object sender, EventArgs e)
        {
            ...
            if (dt.Rows.Count == 1)
            {
                if (dt.Rows[0]["POLINEID"].ToString() == "")
                {
                    ...
                    dt.Rows[0]["POLINENUM"] = "1";
                    lblpolineid.Text = "-1";
                    lblrowindex.Text = "0";
                    txtDPOLineNum.Text = "1";
                    grd.EditIndex = 0;
            ...
            //lblpolineid.Text = dr["POLINEID"].ToString();
            lblrowindex.Text = grd.EditIndex.ToString();
            mydiv.Visible = true;
            txtDCostCode.Text = "";
        ...
        protected void grd_RowEditing(object sender, GridViewEditEventArgs e)
        {
            ...
            lblrowindex.Text = e.NewEditIndex.ToString();
            lblpolineid.Text = dr["POLINEID"].ToString();
            ...
            txtDRequestedBy.Text = dr["REQUESTEDBY"].ToString();// = requestedby;
            txtDTAXCODE.Text = dr["TAXCODE"].ToString();// = taxcode;
            txtDTotalTax.Text = dr["TAXTOTAL"].ToString();// = taxcode;
            txtDRemarks.Text = dr["REMARKS"].ToString(); //= remarks;
            txtReceiptCompleted.Text = (dr["RECEIPT"].ToString());// == "" ? false : (dr["RECEIPT"].ToString() == "No" ? false : true));
            txtQuantityReceived.Text = (dr["RECEIVED"].ToString());// == "" ? false : (dr["RECEIVED"].ToString() == "No" ? false : true));
            chkDTAXExempt.Checked = (dr["TAXED"].ToString() == "" ? false : (dr["TAXED"].ToString() == "No" ? false : true));//= receipt;
            bindGrid(dt);
        ...
        private DataTable createtable()
        {
            ...
            dt.Columns.Add(new DataColumn("ERR", typeof(string)));
        ...
        private void SetPoLines(string CostCode, string POType, string CatalogCode, string Description, string Quantity, string Unit, string UnitPrice, string TotalPrice, string ActionTaken, string PoLineID, string POLINENUM, string model, string brand, string requestedby, string remarks, string taxcode, string totaltax, string taxed, string receipt, string recieved, string requestedName)
        {
            DataRow dr = null;
        ...
        protected void EditPoLines(string CostCode, string POType, string CatalogCode, string Description, string Quantity, string Unit, string UnitPrice, string TotalPrice, DataTable table, string ActionTaken, string PoLineID, string POLINENUM, string model, string brand, string requestedby, string remarks, string taxcode, string totaltax, string taxed, string receipt, string recieved, string RequestedByName)
        {
            ...
            try
            {
                ...
                if (grp.Count > 0)
                {
                    foreach (var g in grp)
                    {
                        ...
                        if (g.RECEIVEDQTY != null)
                        {
                            Recieved = (g.RECEIVEDQTY == null ? "" : g.RECEIVEDQTY.ToString());
                        }
                        if (g.TAXED != null)
                        {
                            Taxed = (g.TAXED == null ? "" : g.TAXED.ToString());
                        }
                        if (g.TAXCODE != null)
                        {
                            TaxCode = g.TAXCODE;
                        ...
                        if (g.TAXTOTAL != null)
                        {
                            TotalTax = (g.TAXTOTAL == null ? "" : g.TAXTOTAL.ToString());
                        }
                        ...
                        if (dt != null)
                        {
                            ...
                            {
                                EditPoLines(g.COSTCODE, g.LINETYPE, g.CATALOGCODE, g.DESCRIPTION, ReturnValue(g.ORDERQTY.ToString()), OrderUnit, ReturnValue(g.UNITCOST.ToString()), ReturnValue(g.LINECOST.ToString()), dt, "", g.POLINEID.ToString(), g.POLINENUM.ToString(), Model, Brand, RequestedBy, Remarks, TaxCode, TotalTax, Taxed, "", Recieved, RequestedByName);
                            }
                        }
                        else
                        {
        ...
        protected void imgPoDelete_Click(object sender, ImageClickEventArgs e)
        {
            try
            {
                ...
                if (Convert.ToInt16(dr["POLINEID"].ToString()) < 0)
                {
        ...
        public string SavePOLines(int PoNum, short Revision)
        {
            ...
            try
            {
                ...
                foreach (DataRow dr in dt.Rows)
                {
                    ...
                    long gvHIdPoLineID = long.Parse(dr["POLINEID"].ToString());
                    int gvHidPOLINENUM = (dr["POLINENUM"] == DBNull.Value ? 1 : int.Parse(dr["POLINENUM"].ToString())); //long.Parse(dr["POLINENUM"].ToString());
                   ...
                   string TAXCODE = dr["TAXCODE"].ToString();// = taxcode;
                    ...
                    bool TAXED = (dr["TAXED"].ToString() == "" ? false : (dr["TAXED"].ToString() == "No" ? false : true));//= receipt;
                    ...
                    if (txtgvPODescription.Trim() == "" || txtgvPostCode.Trim() == null)
                    {
                        return "1086";
                    }


                    if (lblPurchaseActionTaken == "Delete")
                    {
                        value = 1;
                    ...
                    { // if row not delete its either added or updated
                        try
                        {
                            int? PoLineID = 0;
                        ...
                        if (gvHIdPoLineID < 0)
                        {
                            ...
                            try
                            {
                                var Masg = db.PO_AddPOLine(PoNum, Revision, short.Parse(indexValue.ToString()), ddlLineType, ReturnValue(txtgvCATALOGCODE), ReturnValue(txtgvPostCode), ReturnValue(txtgvPODescription),
                                    txtgvOQtn, txtgvPOUnit, txtgvPOUnitPrice, txtgvPOUnitTotal, null, ModelNum, DecRECEIVEDQTN, RejectedQTN, requestedBy,null, Brand, null, Remarks, TAXCODE, DecTAXTOTAL, TAXED, null,null,null, UserName, DateTime.Now, false);
                            }
                            catch (SqlException ex)
                            {
                                return ex.Message;
                            ...
                            if (value == 1)
                            {
                                ...
                                if (ObjPoLine != null)
                                {
                                    ...
                                    try
                                    {
                                        ...
                                        if (ObjPoLine.COSTCODE != txtgvPostCode.Trim())
                                        {
                                            value = 1;
                                        }

                                        if (ObjPoLine.ORDERQTY != txtgvOQtn)
                                        {
                                            value = 1;
                                        }

                                        if (ObjPoLine.ORDERUNIT != txtgvPOUnit.Trim())
                                        {
                                            value = 1;
                                        }

                                        if (ObjPoLine.UNITCOST != txtgvPOUnitPrice)
                                        {
                                            value = 1;
                                        }
                                        if (ObjPoLine.LINECOST != txtgvPOUnitTotal)
                                        {
                                            value = 1;
                                        }
                                        if (ObjPoLine.DESCRIPTION != txtgvPODescription)
                                        {
                                            value = 1;

                                        }
                                        if (ObjPoLine.LINETYPE != ddlLineType)
                                        {
                                            value = 1;
                                            ObjPoLine.LINETYPE = ddlLineType;
                                        }
                                        if (ObjPoLine.CATALOGCODE != txtgvCATALOGCODE.Trim())
                                        {
                                            value = 1;
                                        }
                                        try
        ...
        protected void txtgvPOLineDescriptionEdit_TextChanged(object sender, EventArgs e)
        {
            TextBox edit = (TextBox)sender;
            GridViewRow gvrow = (GridViewRow)edit.Parent.Parent;
            //GridView Grid = (GridView)gvrow.NamingContainer;
        ...
        protected void txtDRequestedBy_TextChanged(object sender, EventArgs e)
        {
            ...
            int rowIndex = int.Parse(lblrowindex.Text);
            FillFeildsinTable("REQUESTEDBY", rowIndex, edit.Text, null);
        }
        ...
        protected void txtDTAXCODE_TextChanged(object sender, EventArgs e)
        {
            ...
            if (txtDTAXCODE.Text != "")
            {
                string TAXCODE = Proj.ValidateFromDomainTableValue("TAXCODE", txtDTAXCODE.Text);
                if (TAXCODE != "")
                {
                    HidTAXCODE.Value = TAXCODE;
                    ClearError();
                    txtDTAXCODE.CssClass = "form-control";
                    upError.Update();
                }
                else
                {
        ...
        protected void chkDTAXExempt_CheckChanged(object sender, EventArgs e)
        {
            ...
            int rowIndex = int.Parse(lblrowindex.Text);
            FillFeildsinTable("TAXED", rowIndex, (edit.Checked ? "Yes" : "NO"), txtDTotalTax);
        }
        public void LoadSignature(string OrgCode)
        {
            try
            {
                if (OrgCode != "")
                {
                    DSSignature.SelectCommand = "Select * from ProjectPOSignatureTemplates where OrgCode='" + OrgCode + "'";
                }
        ...
        protected void LoadAllSignature()
        {
            string Query1 = "SELECT * FROM [ProjectPOSignatureTemplates]  where OrgCode='" + HIDOrganizationCode.Value + "'"; 
            DSLoadAllPOSignature.SelectCommand = Query1 ;
            gvPopUpPOSignature.DataSource = DSLoadAllPOSignature;
            gvPopUpPOSignature.DataBind();
        }
        protected void LoadAllSignatureTemplates()
        {
            string Query1 = "SELECT * FROM [ProjectPOSignature] where PONum='" + txtSignaturePONum.Text + "' AND POREVISION ='" + txtSignaturePORevision.Text + "' AND OrgCode='" + HIDOrganizationCode.Value + "'";
        ...
        protected void btnSaveSignature_Click(object sender, EventArgs e)
        {
            for (int i = 0; i < gvPopUpPOSignature.Rows.Count; i++)
            {
                ...
                if (isChecked)
                {
                    ProjectPOSignatureTemplate UsrSec = db.ProjectPOSignatureTemplates.SingleOrDefault(x => x.POSignID == int.Parse(lblPOpPageID));
                    if (UsrSec != null)
                    {
                        ProjectPOSignature UsrPOSignature = db.ProjectPOSignatures.SingleOrDefault(x => x.OrgCode == HIDOrganizationCode.Value && x.OrderNo == UsrSec.OrderNo && x.PONum == decimal.Parse(txtSignaturePONum.Text) && x.POREVISION == Int16.Parse(txtSignaturePORevision.Text));

                        if (UsrPOSignature == null)
                        {
                            UsrPOSignature = new ProjectPOSignature();
                            UsrPOSignature.CreatedBy = UserName;
                            UsrPOSignature.CreationDateTime = DateTime.Now;
                            UsrPOSignature.Heading = UsrSec.Heading;
                            UsrPOSignature.OrderNo = UsrSec.OrderNo;
                            UsrPOSignature.OrgCode = UsrSec.OrgCode;
                            UsrPOSignature.OrgName = UsrSec.OrgName;
                            UsrPOSignature.PONum = decimal.Parse(txtSignaturePONum.Text);
                            UsrPOSignature.POREVISION = Int16.Parse(txtSignaturePORevision.Text);
                            UsrPOSignature.OrgName = UsrSec.OrgName;
                            UsrPOSignature.TeamMemberCode = UsrSec.TeamMemberCode;
                            UsrPOSignature.TeamMemberName = UsrSec.TeamMemberName;
                            UsrPOSignature.Title = UsrSec.Title;
                            db.ProjectPOSignatures.InsertOnSubmit(UsrPOSignature);
                            db.SubmitChanges();
                            lblSignatureError2.Text = smsg.getMsgDetail(1107);
