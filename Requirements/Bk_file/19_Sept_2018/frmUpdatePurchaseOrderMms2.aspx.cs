...
{
    public partial class frmUpdatePurchaseOrderMms2 : System.Web.UI.Page
    {
        ...
        public void LoadAllPO(int POnum, short Porevision)
        {
            try
            {
                //SrvPOLine.WebSrvPolineSoapClient cs = new SrvPOLine.WebSrvPolineSoapClient();
                //cs.getData2(POnum, Porevision);

                List<WebSrvPoline.ShortPOLINE> SessionPOLine = ((List<WebSrvPoline.ShortPOLINE>)HttpContext.Current.Session["POLINE"]);
                gvPoLInes.DataSource = SessionPOLine;
                gvPoLInes.DataBind();
        ...
        public void LoadPurchaseOrderInformation()
        {
            try
            {
                ...
                if (Request.QueryString["ID"] != null)
                {
                    ...
                    Session["QSPONUM"] = PoNum;
                    Session["QSPORevision"] = revision;
                    LoadAllPO(int.Parse(PoNum), short.Parse(revision));
                    if (ObjgetPo != null)
                    {
                        ...
                        {
                            lblPoNumber.Text = "DRAFT";
                            txtPolinesPurchaseOrderNumber.Text = "DRAFT";
                            txtAttachmentPurchaseOrderNumber.Text = "DRAFT";
                        }
                        ...
                        {
                            txtLessAmount.Text = Convert.ToDecimal(ObjgetPo.DISCOUNT.ToString()).ToString("#,##0.0000");
                        }
                        txtAdditionalChargesDescription.Text = ObjgetPo.ADDCHARGESDESC;
                        if (ObjgetPo.ADDCHARGES != null)
                        {
                            txtAdditionalChargesAmount.Text = Convert.ToDecimal(ObjgetPo.ADDCHARGES.ToString()).ToString("#,##0.0000");
                        }
                        ...
                        {
                            txtPolinePOSubCost.Text = Convert.ToDecimal(ObjgetPo.SUBTOTALCOST).ToString("#,##0.0000");
                        }
                        if (ObjgetPo.TOTALCOST != null)
                        {
                            txtPOLinesPurchaseOrderTotalCost.Text = Convert.ToDecimal(ObjgetPo.TOTALCOST).ToString("#,##0.0000");
                        }
                        ...
                        //LoadAllPoLines(ObjgetPo.PONUM, ObjgetPo.POREVISION); mms LoadPoLine

                        //LoadAllAttachments

                        DataTable dtTest = new DataTable();
                        Session["Attachment"] = dtTest;
                        LoadAllAttachment(ObjgetPo.PONUM);
                        LockControl(ObjgetPo.PONUM, ObjgetPo.POREVISION);
                        if (ObjgetPo.POREF != null)
                        {
        ...
        protected void LoadAllAttachment(decimal PurchaseOrderID)
        {
            try
            {
                ResetLabel();
        ...
        //    gvPoLInes.DataBind();*/
        //    DataTable table = (DataTable)ViewState["PoLines"];
    ...
    /*    private void SetPoLines(string CostCode, string POType, string CatalogCode, string Description, string Quantity, string Unit, string UnitPrice, string TotalPrice, string ActionTaken, string PoLineID, string POLINENUM)
        {
            ...
            if (Quantity != "")
            {
                dr["Quantity"] = Convert.ToDecimal(Quantity).ToString("#,##0.0000");// Quantity;
            }
            else
            {
            ...
            if (UnitPrice != "")
            {
                dr["UnitPrice"] = Convert.ToDecimal(UnitPrice).ToString("#,##0.0000");  //UnitPrice;
            }
            else
            {
            ...
            if (UnitPrice != "")
            {
                dr["TotalPrice"] = Convert.ToDecimal(TotalPrice).ToString("#,##0.0000"); ; //TotalPrice.ToString("N4");
            }
            else
            {
                        ...
                        for (int i = 1; i <= dtCurrentTable.Rows.Count; i++)
                        {
                            ...
                            if (txtGvTotalPrice.Text == "")
                            {
                                if (txtGvQuantity.Text != "" && txtGvPrice.Text != "")
                                {
                                    ...
                                    txtGvTotalPrice.Text = TotalCost.ToString("#,##0.0000");
                                    value = TotalCost;
                                }
                            }
                ...
                if (grp.Count > 0)
                {
                    foreach (var g in grp)
                    {
                        ...
                        if (dt != null)
                        {
                            if (dt.Rows.Count == 0)
                            {
                                SetPoLines(g.COSTCODE, g.LINETYPE, g.CATALOGCODE, g.ITEMDESCRIPTION, ReturnValue(g.ORDERQTY.ToString()), OrderUnit, ReturnValue(g.UNITCOST.ToString()), ReturnValue(g.LINECOST.ToString()), "", g.POLINEID.ToString(), g.POLINENUM.ToString());
                            }
                            else
                            {
                                EditPoLines(g.COSTCODE, g.LINETYPE, g.CATALOGCODE, g.ITEMDESCRIPTION, ReturnValue(g.ORDERQTY.ToString()), OrderUnit, ReturnValue(g.UNITCOST.ToString()), ReturnValue(g.LINECOST.ToString()), dt, "", g.POLINEID.ToString(), g.POLINENUM.ToString());
                            }
                        }
                        else
                        {
                ...
                if (Request.QueryString["ID"] != null)
                {
                    ...
                    if (ObjPO != null)
                    {
                        ...
                        using (TransactionScope trans = new TransactionScope())
                        {
                            if (i == 1)
                            {
                                try
                                {
                                    ...
                                    var Masg = db.PO_EditPO(ObjPO.PONUM, ObjPO.POREF, ObjPO.POSID, PORevision, ReturnValue(Description), HIDOrganizationCode.Value, txtOrganization.Text, HidProjectCode.Value, txtProjectCode.Text,
                                        ReturnValue(txtRequistionRefNum.Text), ReturnValue(txtQuotationRef.Text), dtQuotationDate, ReturnValue(txtPaymentTerms.Text.Trim()), dtOrderDate, dtRequiredDate, dtVendorDate, HidPOType.Value, ReturnValue(txtOriginalPO.Text), HidBuyersID.Value, txtBuyers.Text, int.Parse(txtCompanyID.Text), ReturnValue(txtCompanyName.Text), ReturnValue(txtCompanyAddress.Text),
                                    ...
                                    ReturnValue(txtDeliverContact2Mobile.Text), ReturnValue(txtDeliverContact2Position.Text), SubTotal, Discount, ReturnValue(txtLessDescription.Text), AdditionalCharges, ReturnValue(txtAdditionalChargesDescription.Text), TotalCost, UserName, ContractRef, StatusCode, DateTime.Parse(txtStatusDate.Text),null,null, false);
                            ...
                            //mms POLine need to right the save code 
                            string confirmPoLines = SavePOLines(ObjPO.PONUM, ObjPO.POREVISION);
                            ...
                            string value = UpdatePurchaseOrderAttachment(ObjPO.PONUM);
                            if (value != "noChange")
                            {
                                if (value != "Success")
                                {
        ...
        protected string UpdatePurchaseOrderAttachment(decimal PONum)
        {
            ResetLabel();
            int value = 0;
            ...
            try
            {
                ...
                foreach (GridViewRow item in gvShowSeletSupplierAttachment.Rows)
                {
                    ...
                    if (lblSupplierActionTaken.Text == "Update")
                    {
                        try
                        {
                            var Masg = db.sp_update_Attachment(int.Parse(HidAttachmentID.Value), int.Parse(PONum.ToString()), "PO", Title, Description, VarFile.Name, VarFile.Length.ToString(), VarFile.Extension, lblSupplierAttachmentFileURL.Text, "INT", UserName, DateTime.Now, false);
                        }
                        catch (SqlException ex)
                        {
                            return ex.Message;
                    ...
                    else if (lblSupplierActionTaken.Text == "New")
                    {
                        ...
                        try
                        {
                            var Masg = db.sp_add_Attachment(int.Parse(PONum.ToString()), "PO", Title, Description, VarFile1.Name, VarFile1.Length.ToString(), VarFile1.Extension, DestinationFile, "INT", UserName, DateTime.Now, false);
                        }
                        catch (SqlException ex)
                        {
                            return ex.Message;
                    ...
                    else if (lblSupplierActionTaken.Text == "Delete")
                    {
                        ...
                        if (atc != null)
                        {
                            try
                            {
                                var Masg = db.sp_delete_Attachment(atc.AttachmentID, int.Parse(PONum.ToString()), "PO", UserName, DateTime.Now, false);
                            }
                            catch (SqlException ex)
                            {
                                return ex.Message;
        ...
        public string SavePOLines(decimal PoNum, short Revision)
        {
            int value = 0;
            string returnmasg;
            try
            { 
                List<WebSrvPoline.ShortPOLINE> SessionPOLine = ((List<WebSrvPoline.ShortPOLINE>)HttpContext.Current.Session["POLINE"]);
                for (int i = 0; i < SessionPOLine.Count; i++)
                {
                    Int16 POLineNumber = Int16.Parse(SessionPOLine[i].POLINENUM.ToString());
                    var chckPO = db.POLINEs.Where(x => x.PONUM == PoNum && x.POREVISION == Revision && x.POLINENUM == POLineNumber);
                     if (chckPO != null)
                     {
                         string action = SessionPOLine[i].Actions;
                         if (action == "Delete")
                         {
                             try
                             {
                                 value = 1;
                                 var msf = db.PO_DeletePOLine(PoNum, Revision, POLineNumber, UserName, DateTime.Now, false);
                             }
                             catch (SqlException ex)
                             {
                                 return ex.Message;
                         ...
                         {
                             int j = 0;
                             WebSrvPoline.ShortPOLINE Values = SessionPOLine.Find(Product => Product.POLINENUM == POLineNumber);
                             if (Values != null)
                             {
                                 foreach (var k in chckPO)
                                 {
                                     if (ReturnValue(Values.CATALOGCODE) != k.CATALOGCODE)
                                     {
                                         j = 1;
                                     }
                                     if (ReturnValue(Values.COSTCODE) != k.COSTCODE)
                                     {
                                         j = 1;
                                     }
                                     if (ReturnValue(Values.ITEMDESCRIPTION) != k.DESCRIPTION)
                                     {
                                         j = 1;
                                     }
                                     if (Values.LINECOST != k.LINECOST)
                                     {
                                         j = 1;
                                     }
                                     if (Values.LINETYPE != k.LINETYPE)
                                     {
                                         j = 1;
                                     }
                                     if (Values.ORDERQTY != k.ORDERQTY)
                                     {
                                         j = 1;
                                     }
                                     if (ReturnValue(Values.ORDERUNIT) != k.ORDERUNIT)
                                     {
                                         j = 1;
                                     }
                                     if (Values.UNITCOST != k.UNITCOST)
                                     {
                                         j = 1;
                                     }
                                     if (j == 1)
                                     {
                                         try
                                         {
                                             if (Values.UNITCOST != null & Values.ITEMDESCRIPTION != null)
                                             {
                                                 value = 1;
                                                 var Masg = db.PO_EditPOLine(PoNum, Revision, POLineNumber, Values.LINETYPE, ReturnValue(Values.CATALOGCODE), ReturnValue(Values.COSTCODE), ReturnValue(Values.ITEMDESCRIPTION),
                                              Values.ORDERQTY, ReturnValue(Values.ORDERUNIT), Values.UNITCOST, Values.LINECOST, null, null, null, null, null, null, null, null, null, null, null, null, null, UserName, DateTime.Now, false);
                                             }
                                         }
                                         catch (SqlException ex)
                                         {
                         ...
                         {
                             WebSrvPoline.ShortPOLINE Values = SessionPOLine.Find(Product => Product.POLINENUM == POLineNumber);
                             if (Values.ITEMDESCRIPTION.Trim() == "" || Values.LINECOST == null || Values.ITEMDESCRIPTION.Trim() == null)
                             {
                                 return "Can't null";
                             }
                             else
                             {
                                 ...
                                 {
                                     value = 1;
                                 var Masg = db.PO_AddPOLine(PoNum, Revision, POLineNumber, Values.LINETYPE, ReturnValue(Values.CATALOGCODE), ReturnValue(Values.COSTCODE), ReturnValue(Values.ITEMDESCRIPTION),
                                          Values.ORDERQTY, ReturnValue(Values.ORDERUNIT), Values.UNITCOST, Values.LINECOST,null, null, null, null,null, null, null, null, null, null, null, null, null, "mms", DateTime.Now, false);
                                 }
                                 catch (SqlException ex)
                                 {
                                     return ex.Message;
        ...
        //                    try
        //                    {
        //                        var Masg = db.PO_AddPOLine(PoNum, Revision, short.Parse(indexValue.ToString()), ddlLineType.SelectedValue, ReturnValue(txtgvCATALOGCODE.Text), ReturnValue(txtgvPostCode.Text), ReturnValue(txtgvPODescription.Text),
        ...
        //                            }
        //                        }
        //                        if (ObjPoLine.ITEMDESCRIPTION != txtgvPODescription.Text)
        //                        {
        //                            value = 1;
        ...
        //                {
        //                    if (ObjCheckPoLine.ITEMDESCRIPTION != txtgvPODescription.Text)
        //                    {
        //                        UpdatValue = 1;
        ...
        //                txtPOUnitPrice.Text = UnitPrice.ToString("#,##0.0000");
        //            }
        ...
        //                txtPOQtn.Text = Qtn.ToString("#,##0.0000");
        //            }
        //            else if ((txtPOQtn.Text.Trim() == "" && txtPOUnitPrice.Text.Trim() != "0") || (txtPOQtn.Text.Trim() == "" && txtPOUnitPrice.Text.Trim() != ""))
        //            {
        ...
        //                txtPOUnitTotal.Text = totalcount.ToString("#,##0.0000");
        //            }
        ...
        //                    txtPOUnitPrice.Text = Qtn.ToString("#,##0.0000");
        //                }
        //            }
        //        }
        ...
        //                txtPOQtn.Text = totalcount.ToString("#,##0.0000");
        //            }
        //        }
        ...
        */
        ...
        public void LockControl(decimal PoNum, int Revisionid)
        {
            try
            {
      ...
      /*  protected void ddlLineType_TextChanged(object sender, EventArgs e)
        {
            ...
            int Value = TotalCalculation("POType", rowIndex.ToString());
            if (Value == 1)
            {
            }
        }
        ...
        protected void txtPOUnitPrice_TextChanged(object sender, EventArgs e)
        {
            ...
            int rowIndex = gvrow.RowIndex;
            int value = TotalCalculation("UnitPrice", rowIndex.ToString());
        }
        ...
        protected void txtPOCostCode_TextChanged(object sender, EventArgs e)
        {
            ...
            int rowIndex = gvrow.RowIndex;
            int Value = TotalCalculation("CostCode", rowIndex.ToString());
            if (Value == 1)
            {
            }
        }
        ...
        protected void txtPOQtn_TextChanged(object sender, EventArgs e)
        {
            ...
            int rowIndex = gvrow.RowIndex;
            int Value = TotalCalculation("QTN", rowIndex.ToString());
            if (Value == 1)
            {
            }
        }
        protected void txtPOUnitTotal_TextChanged(object sender, EventArgs e)
        {
            ...
            int rowIndex = gvrow.RowIndex;
            int value = TotalCalculation("TotalPrice", rowIndex.ToString());
        }
        ...
        protected void txtgvCATALOGCODE_TextChanged(object sender, EventArgs e)
        {
            ...
            GridViewRow gvrow = (GridViewRow)edit.Parent.Parent;
            int rowIndex = gvrow.RowIndex;
            int Value = TotalCalculation("CatalogCode", rowIndex.ToString());
            if (Value == 1)
            {
            }
        }
        ...
        protected void gvPoLInes_RowDataBound1(object sender, GridViewRowEventArgs e)
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                short PolineID = short.Parse(gvPoLInes.DataKeys[e.Row.RowIndex].Value.ToString());

                List<WebSrvPoline.ShortPOLINE> SessionPOLine = ((List<WebSrvPoline.ShortPOLINE>)HttpContext.Current.Session["POLINE"]);
                if(SessionPOLine != null)
                {
                    var ObjPo= SessionPOLine.Where(x=>x.POLINENUM == PolineID);
                    if (ObjPo != null)
                    {
                        foreach (var i in ObjPo)
                        { 
                          // if(i.)
                        }
                    }
