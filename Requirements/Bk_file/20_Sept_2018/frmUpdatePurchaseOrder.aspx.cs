...
namespace FibrexSupplierPortal.Mgment
{
    ...
    public partial class frmUpdatePurchaseOrder : System.Web.UI.Page
    {
        ...
        protected void btnAddLines_Click(object sender, EventArgs e)
        {
            ...
            if (dt.Rows.Count == 1)
            {
                if (dt.Rows[0]["POLINEID"].ToString() == "")
                {
                    ...
                    dt.Rows[0]["TAXED"] = "True";
                    dt.Rows[0]["ADDEDBY"] = UserName;
                    dt.Rows[0]["ADDEDON"] = DateTime.Now.ToString();
                    lblpolineid.Text = "-1";
                    lblrowindex.Text = "0";
                    ...
                    loadMyDIV(dt.Rows[0], grd.EditIndex);
                }
                else
                {
                    //grd.ShowFooter = true;
                    ...
                    dr["TAXED"] = "True";
                    dr["ADDEDBY"] = UserName;
                    dr["ADDEDON"] = DateTime.Now.ToString();
                    dt.Rows.Add(dr);
        ...
        protected void loadMyDIV(DataRow dr, int rowIndex) {
            ...
            txtDPOLineNum.Text = dr["POLINENUM"].ToString();
            txtDCostCode.Text = dr["CostCode"].ToString();
            ddlDLineType.SelectedValue = dr["POType"].ToString();
            txtDCatalogCode.Text = dr["CATALOGCODE"].ToString();
            txtDItem.Text = dr["Description"].ToString();
            ...
            txtDDTAXCODE.Text = dr["TAXCODE"].ToString();// = taxcode;
        ...
        protected void gvITEMCODE_RowCommand(object sender, ASPxGridViewRowCommandEventArgs e)
        {
            ...
            txtDItem.Text = ITEMDESC;
            txtDUOM.Text = UNIT;

            txtDItemCode.ReadOnly = txtDesc.ReadOnly = true;
            txtDItem.ReadOnly = txtUnit.ReadOnly = true;
            txtDUOM.ReadOnly = txtCC.ReadOnly = true;
        ...
        private void SetPoLines(string CostCode, string POType, string CatalogCode, string ITEMCODE, string Description, string Quantity, string Unit, string UnitPrice, string TotalPrice, string ActionTaken, string PoLineID, string POLINENUM, string model, string brand, string requestedby, string requestedName, string remarks, string taxcode, string totaltax, string taxed, string receipt, string recieved, string createdby, string createdon, string editedby, string editedon)
        {
            DataRow dr = null;
            ...
            try
            {
                ...
                if (grp.Count > 0)
                {
                    foreach (var g in grp)
                    {
                        ...
                        {
                            AddedON = g.LASTMODIFIEDDATE.ToString();
                        }
                        if (g.ITEMNUM != null)
                        {
                            ItemCode = g.ITEMNUM;
                        ...
                        if (g.TAXED != null)
                        {
                            TaxExempted = g.TAXED.ToString();// (g.TAXED == true ? "" : g.TAXED.ToString());
                        }
                        if (g.TAXCODE != null)
                        {
                            Taxcode = g.TAXCODE;
                        ...
                        if (dt != null)
                        {
                            if (dt.Rows.Count == 0)
                            {
                                SetPoLines(CostCode, Linetype, CatalogCode, ItemCode, Description, Quantity, Unit, UnitCost, LineCost, "", g.POLINEID.ToString(), g.POLINENUM.ToString(), Model, Brand, RequestedBy, RequestedByName, Remarks, Taxcode, TaxAmount, TaxExempted, Recieved, Recieved ,AddedBY, AddedON, EditedBY, EditedON);
                            }
                            else
                            {
                                EditPoLines(dt,CostCode, Linetype, CatalogCode, ItemCode, Description, Quantity, Unit, UnitCost, LineCost, "", g.POLINEID.ToString(), g.POLINENUM.ToString(), Model, Brand, RequestedBy, RequestedByName, Remarks, Taxcode, TaxAmount, TaxExempted, Recieved, Recieved ,AddedBY, AddedON, EditedBY, EditedON);
                            }
                        }
                        else
                        {
                            ...
                            if (i == 1)
                            {
                                try
                                {
                                    ...
                                    var Masg = db.PO_EditPO(ObjPO.PONUM, ObjPO.POREF, ObjPO.POSID, PORevision, ReturnValue(Description), HIDOrganizationCode.Value, txtOrganization.Text, HidProjectCode.Value, txtProjectCode.Text,
                                       ReturnValue(txtRequistionRefNum.Text), ReturnValue(txtQuotationRef.Text), dtQuotationDate, ReturnValue(txtPaymentTerms.Text.Trim()), dtOrderDate, dtRequiredDate, dtVendorDate, HidPOType.Value, ReturnValue(txtOriginalPO.Text), HidBuyersID.Value, ReturnValue(txtBuyers.Text), int.Parse(txtCompanyID.Text), ReturnValue(txtCompanyName.Text), ReturnValue(txtCompanyAddress.Text),
        ...
        public string SavePOLines(int PoNum, short Revision)
        {
            ...
            try
            {
                ...
                foreach (DataRow dr in dt.Rows)
                {
                   ...
                   // long gvHIdPoLineID = long.Parse(dr["POLINEID"].ToString());Rizwan
                    ...
                    if (txtgvPODescription.Trim() == "" || txtgvPOUnitTotal == null) //|| txtITEMCODE.Trim() == "" || txtITEMCODE.Trim() == null
                    {
                        return "1086";
                    }
                        ...
                        if (gvHIdPoLineID < 0)
                        {
                            ...
                            try
                            {
                                var Masg = db.PO_AddPOLine(PoNum, Revision, short.Parse(indexValue.ToString()), ddlLineType, ReturnValue(txtgvCATALOGCODE), ReturnValue(txtgvPostCode), ReturnValue(txtgvPODescription),
                                    txtgvOQtn, txtgvPOUnit, txtgvPOUnitPrice, txtgvPOUnitTotal, txtITEMCODE, ModelNum, requestedBy, requestName, Brand, Remarks, TAXCODE, DecTAXTOTAL, TAXED,null, UserName, DateTime.Now, false);
                            }
                            catch (SqlException ex)
                            {
                                return ex.Message;
        ...
        void CalculateTotalCost(DataTable dt)
        {
            ...
            txtPolinePOSubCost.Text = res.ToString();
        }
        ...
        public void FillFeildsinTable(string FieldName, int Row, string Value, TextBox lbl)
        {
            ...
            dr["ActionTaken"] = "";// if any deleted row is edited... 

            dr["EDITEDBY"] = UserName;
            dr["EDITEDON"] = DateTime.Now.ToString();
            bindGrid2(dt);
        ...
        {
            string Query1 = "SELECT * FROM [POSignatureTemplates]  where OrgCode='" + HIDOrganizationCode.Value + "'";
        ...
        protected void btnSaveSignature_Click(object sender, EventArgs e)
        {
            if (HidSignatureAction.Value == "Update")
            {
                ...
                if (UsrPOSignature != null)
                {
                    try
                    {
                        var masg = db.PO_EDITPOSignature(HIDOrganizationCode.Value, txtOrganization.Text, int.Parse(HIDSignatureUpdateSIGNID.Value), int.Parse(txtEditSignatureOrder.Text), decimal.Parse(txtSignaturePONum.Text), Int16.Parse(txtSignaturePORevision.Text), txtEditSignatureHeading.Text, txtEditSignatureTitle.Text, ddlEditSignature.SelectedItem.Value.ToString(), ddlEditSignature.SelectedItem.Text, UserName, true);
            ...
            else
            {
                for (int i = 0; i < gvPopUpPOSignature.Rows.Count; i++)
                {
                    ...
                    if (isChecked)
                    {
                        POSignatureTemplate UsrSec = db.POSignatureTemplates.SingleOrDefault(x => x.POSignID == int.Parse(lblPOpPageID));
                        if (UsrSec != null)
                        {
                            ...
                            if (UsrPOSignature == null)
                            {
                                try
                                {
                                    var masg = db.PO_ADDPOSignature(UsrSec.OrgCode, null, UsrSec.OrderNo, decimal.Parse(txtSignaturePONum.Text), Int16.Parse(txtSignaturePORevision.Text), UsrSec.Authority, UsrSec.Title,null,null, UserName, true);
                                }
                                catch (SqlException ex)
                                {
                                    lblSignatureError2.Text = ex.Message;
        ...
        protected void lnkSignatureEditOptin_Click(object sender, EventArgs e)
        {
            ...
            if (POSignature != null)
            {
                txtEditSignatureHeading.Text = POSignature.Heading;
                txtEditSignatureOrder.Text = POSignature.OrderNo.ToString();
                txtEditSignatureTitle.Text = POSignature.Title;
                HIDSignatureUpdateSIGNID.Value = POSignature.POSignID.ToString();
                string ID = POSignature.TeamMemberCode;
                ddlEditSignature.Value = ID;
        ...
        public void LoadPoppSignatureUser()
        {
            try
            {
                ddlEditSignature.DataSource = db.FIRMS_GetAllEmployee();
                ddlEditSignature.DataBind();
