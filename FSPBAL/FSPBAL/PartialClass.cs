...
namespace FSPBAL
{
    ...
    {
        FSPDataAccessModelDataContext db = new FSPDataAccessModelDataContext(ConfigurationManager.ConnectionStrings["CS"].ToString());
        public string SaveRecordInAuditProjectMember(string userID, string Action, string orgCode,  int PODefinationID)
        {
            string masg = string.Empty;
            try
            {
                PODefination ObjTeam = db.PODefinations.FirstOrDefault(x => x.OrgCode == orgCode && x.PODefinationID == PODefinationID);
